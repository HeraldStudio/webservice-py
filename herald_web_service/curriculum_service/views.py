...
def curriculum(request, cardNumber, academicYear):
    params = urllib.urlencode(
        {'queryStudentId': cardNumber, 'queryAcademicYear': academicYear})
    uFile = urllib.urlopen(
        "http://xk.urp.seu.edu.cn/jw_service/service/stuCurriculum.action", params)
    ...
    if match:
            ...
            for j in i:
                for k in j:
                    if len(k) == 2:
                        k.append('')
                    try:
                        k.insert(1, [k[1].split(']')[0][1:-1].split('-')[0], k[1].split(']')[0][1:-1].split('-')[1]])
                        k[2] = [k[2].split(']')[1][:-1].split('-')[0], k[2].split(']')[1][:-1].split('-')[1]]
                    except:
                        k.insert(1, [])
                        k[2] = []
        saturdayCourse = []
        ...
        returnList = []
        for i in xrange(5):
            returnList.append(courseList[0][i] + courseList[1][i] + courseList[2][i])

        returnList.append(saturdayCourse)
        returnList.append(sundayCourse)

        return HttpResponse(json.dumps([sidebarList, returnList], ensure_ascii=False))
